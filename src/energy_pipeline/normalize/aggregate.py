...
def _enforce_eu_scope(df: pd.DataFrame) -> pd.DataFrame:
    """
    EU-only:
      - keep only EU27 countries where `country` may be:
          * ISO2 code (DE, AT, ...)
          * display name (Germany, Austria, ...)
      - normalize output to display name for nicer dashboard
      - force region="EU"
    """
    out = df.copy()

    iso2_allowed = set(EU27_ISO2)
    name_allowed = set(EU27_NAMES.values())

    if "country" in out.columns:
        c = out["country"].astype(str).str.strip()

        # keep rows where country is EU ISO2 OR EU display name
        keep_mask = c.isin(iso2_allowed) | c.isin(name_allowed)
        out = out[keep_mask].copy()

        # normalize ISO2 -> display name (if possible)
        out["country"] = out["country"].astype(str).str.strip().map(lambda x: EU27_NAMES.get(x, x))
